...
rm /root/gestorssh.zip > /dev/null 2>&1
rm $HOME/gestorssh.zip > /dev/null 2>&1
rm /root/ssh.sql > /dev/null 2>&1
rm $HOME/ssh.sql > /dev/null 2>&1
####################################################################################################
echo -e "\E[44;1;37m    INSTALAR O PAINELWEB SSH/DROP/SSL MultiHD-SSH     \E[0m"
echo ""
echo -e "                              \033[1;31mBy @Sr_Wallker\033[1;36m"
echo -e "   MultiHD-SSH" | figlet 
echo -e "                             \033[1;31mPAINELWEB MultiHD-SSH\033[1;36m"
echo ""
echo -ne "\n\033[1;32m  DEFINA UMA SENHA PARA O\033[1;33m MySQL\033[1;37m: "; read senha
echo -e "\n\033[1;36m  INICIANDO INSTALAÇÃO \033[1;33m"
echo ""
...
wget https://github.com/Digulgasoft/v2022-list/2022.zip > /dev/null 2>&1
sleep 1
...
sleep 1
cd
wget https://github.com/Digulgasoft/v2022-list/ssh.sql> /dev/null 2>&1
sleep 1
if [[ -e "$HOME/ssh.sql" ]]; then
    mysql -h localhost -u root -p$senha --default_character_set utf8 sshplus < ssh.sql
    rm /root/ssh.sql
else
    clear
...
echo '* * * * * root /usr/bin/php /var/www/html/pages/system/cron.ssh.teste.php' >> /etc/crontab
...
# BACKUP BANCO DE DADOS DATABASE SQL A CADA 30 MINUTOS #
echo '*/30 * * * * root /bin/autobackup.sh' >> /etc/crontab
/etc/init.d/cron reload > /dev/null 2>&1
/etc/init.d/cron restart > /dev/null 2>&1
cd /bin
rm autobackup.sh > /dev/null 2>&1
wget https://github.com/wellzin-blip/v2022/raw/master/gestorssh/autobackup.sh > /dev/null 2>&1 && chmod 777 autobackup.sh && sed -i -e 's/\r$//' autobackup.sh && ./autobackup.sh
